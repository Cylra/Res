...
name <- "Results--Dingo-nanshan.hu.nornet-ip4_6-1"
...
#      zTitle <- "From Provider{F}"
      zSet   <- sprintf("From:%s--To:%s",allResultsSubset$FromProvider,allResultsSubset$ToProvider)
      zTitle <- ""
   ...
   #xAxisTicks <- getIntegerTicks(xSet)   # Set to c() for automatic setting
      xAxisTicks <- seq(0, 300, 10)   # Set to c() for automatic setting
